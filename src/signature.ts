...
import { Provider, ProviderSignature, WebhookEndpoint, WebhookSignatureData } from "./types";
import env from "./env";
...
export function createSignature(secret: string, data: WebhookSignatureData) {
    const encodedSecret = Buffer.from(secret.slice(env.KELP_SECRET_PREFIX.length), 'base64')
    const dataToBeSigned = `${data.id}.${data.timestamp}.${JSON.stringify(data.data)}`

    return createHmac("sha256", encodedSecret).update(dataToBeSigned).digest("base64")
}
 ...
 */
export function compareSignatures(receivedSignature: string, computedSignature: string) {
 ...
 */
export function verifySignature(data: WebhookSignatureData, signature: string, secrets: string[]): boolean {
