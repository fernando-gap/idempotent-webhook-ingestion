...
import { compareSignatures, createProviders, createSecret, createSignature, verifySignature } from '../src/signature'
import env from '../src/env'
import { ProviderSignature, WebhookSignatureData } from '../src/types.js'
...
describe("createSecret", async () => {
    it("returns a secret prefixed with env.KELP_SECRET_PREFIX", () => {
        ...
        expect(env.KELP_SECRET_PREFIX.endsWith("_"), "The KELP_SECRET_PREFIX must end in an _").toBe(true)
...
describe("verifySignature", async () => {
    ...
    it("returns true when at least one verified provider signatures are true", () => {
        ...
        const providerSignature = 'v1,hP+toWa4ULbyjVB5AxAgFHvaJM863OtpwKGdOAaxKOo= v1,ydnc64SncH19cxaSCIP53tSrpf7qOJXuLJTGFt2fiXA='
        const secrets = ["whsec_O4BPiUPotPzQBa1moMvr2fcd1/YlCf/J2S1bp3WtBVU=", "whsec_ITirpiST4snSMSPuF5bnpztjnW1wDjY4VrnitpGcECA="]
    ...
    it("returns false when all provider signatures are invalid", () => {
        const providerSignature = 'v1,hP+toWa4ULbyjVB5AxAgFHvaJM863OtpwKGdOAaxKOo= v1,ydnc64SncH19cxaSCIP53tSrpf7qOJXuLJTGFt2fiXb='
        const secrets = ["whsec_O4BPiUPotPzQBa1moMvr2fcd1/YlCf/J2S1bp3WtBVU=", "whsec_ITirpiST4snSMSPuF5bnpztjnW1wDjY4VrnitpGcECA="]
        const isValid = verifySignature(data, providerSignature, secrets)
